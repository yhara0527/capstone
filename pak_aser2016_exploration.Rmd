...
anova <- anova(glm_child_dist_null, glm_child_dist, test = "Chisq")
data.frame(DID = dist, chi = anova$`Pr(>Chi)`[2], chi_largerThan_0.05 = anova$`Pr(>Chi)`[2] >= 0.05, chi_largerThan_0.1 = anova$`Pr(>Chi)`[2] >= 0.1)
})
...
anova_dist %>% 
  summarize(total_chi_largerThan_0.05 = sum(as.numeric(chi_largerThan_0.05)),
            ratio = sum(as.numeric(chi_largerThan_0.05))/length(chi_largerThan_0.05))
```
