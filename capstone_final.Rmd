...
child_ica_dummy$PR004ORPR009_01 <- ifelse(
...
child_ica_dummy$PR004ANDPR009_01 <- ifelse(
  child_ica_dummy$PR004 == -1 & child_ica_dummy$PR009 == -1, 1, 0)
```
...
NCH <- child_ica_dummy %>% 
  group_by(HHID) %>% 
  summarize(NCH = length(unique(CID)))

child_ica_dummy <- child_ica_dummy %>% left_join(NCH)
...
#### Hunza. GLM Age >= 5 C003_01 ~ NCH + H003

```{r}
glm_child <- glm(C003_01 ~ NCH + H003, family = "binomial", data = child_ica_dummy %>% filter(C001 >= 5, !is.na(H003), DID == 266))
...
child_ica_dummy %>% 
  ...
  group_by(H003) %>% 
  ggplot(aes(factor(PR004ANDPR009_01), fill = H003)) +
  geom_bar(stat = "count") +
  theme(text = element_text(family = "serif"))
```
...
child_ica_dummy %>% 
  filter(DID == 266) %>% 
  ggplot(aes(factor(NCH), fill = H003)) +
  geom_bar() +
  theme(text = element_text(family = "serif"))
```
```{r}
...
```{r}
child_ica_dummy %>% 
  filter(C001 >= 5, DID == 266, !is.na(H003)) %>% 
  group_by(NCH) %>% 
  summarize(ownership_ratio = mean(H003 == -1)) %>% 
  ggplot(aes(NCH, ownership_ratio)) +
  geom_point() +
  geom_smooth(method = "lm")+
  ylim(c(0,1)) +
  labs(title = "Correlation Between Number of Cihldren and House Hold Ownership?") +
  theme(text = element_text(family = "serif"))
```
...
#### Hunza. GLM Age >= 5 C003_01 ~ NCH + PR004ANDPR009_01

```{r}
glm_child <- glm(C003_01 ~ NCH + PR004ANDPR009_01, family = "binomial", data = child_ica_dummy %>% filter(C001 >= 5, DID == 266))

ggPredict(glm_child, se = TRUE, colorAsFactor = TRUE, show.summary = TRUE, point = TRUE) +
  theme(text = element_text(family = "serif")) +
  xlab("Number of Children in Each House Hold") +
  ylab("Enrollment") +
  labs(color = "Parents' Education (Both)", fill = "Parents' Education (Both)")
...
```{r}
step(glm_child_null, direction = "both", 
     scope = (~ C001 + C002_01 + PR004ANDPR009_01 + PR004ORPR009_01 + NCH))
```
...
```{r}
child_ica_dummy %>% 
  filter(DID == 266, C001 >= 5) %>% 
  ggplot(aes(factor(NCH), fill = factor(PR004ANDPR009_01))) +
...
#### Hunza. GLM Age >= 5 C003_01 ~ NCH + PR004ORPR009_01

```{r}
glm_child <- glm(C003_01 ~ NCH + PR004ORPR009_01, family = "binomial", data = child_ica_dummy %>% filter(C001 >= 5, DID == 266))

ggPredict(glm_child, se = TRUE, colorAsFactor = TRUE, show.summary = TRUE, point = TRUE) +
  theme(text = element_text(family = "serif")) +
  xlab("Number of Children in Each House Hold") +
  ylab("Enrollment") +
  labs(color = "Parents' Education (Either)", fill = "Parents' Education (Either)")
...
#### Each Dist. GLM Age >= 5 C003_01 ~ NCH + H003
...
each_dist <- sapply(as.numeric(as.character(unique(child_ica_dummy$DID))), function(id){
  ...
  glm_child <- glm(C003_01 ~ NCH + H003, 
...
```{r}
child_ica_dummy %>% 
  filter(C001 >= 5, DID %in% DID_HosLem0.05_model_1) %>% 
  group_by(DID, C001, C002) %>% 
  mutate(rate = mean(C003 == 3)) %>% 
  ggplot(aes(C001, rate, color = C002)) +
  geom_line() +
  facet_wrap(.~DNAME) +
  xlab("Age") +
  ylab("Enrollment Rate") +
  labs(title = "Districts Where Model 1 Don't Fit Well", 
       caption = "More detailed analysis will be left for the future research, \n but it is clear that in some districts, gender and age matter.",
       color = "Gender") +
  theme(text = element_text(family = "serif"))
```
...
#### Whole Pak. GLM Age >= 5 C003_01 ~ NCH + PR004ANDPR009_01 + relevel(DID, ref = "266")

```{r}
glm_child <- glm(C003_01 ~ NCH + PR004ANDPR009_01 + relevel(DID, ref = "266"), family = "binomial", data = child_ica_dummy %>% filter(C001 >= 5))
...
#### Each Dist. GLM Age >= 5 C003_01 ~ NCH + PR004ANDPR009_01

```{r}
each_dist <- sapply(as.numeric(as.character(unique(child_ica_dummy$DID))), function(id){
  child_ica_dummy <- child_ica_dummy %>% filter(DID == id)
  glm_child <- glm(C003_01 ~ NCH + PR004ANDPR009_01, 
...
#### Not dists_not_fit. GLM Age >= 5 C003_01 ~ NCH + PR004ANDPR009_01

```{r}
glm_child <- glm(C003_01 ~ NCH + PR004ANDPR009_01, family = "binomial", data = child_ica_dummy %>% filter(C001 >= 5, !DID %in% dists_not_fit))
