...
```{r}
N_Child_HH <- child_ica_dummy %>% 
  group_by(HHID) %>% 
  summarize(N_Child_HH = length(unique(CID)))

child_ica_dummy <- child_ica_dummy %>% left_join(N_Child_HH)
...
#### Hunza. GLM Age >= 5 C003_01 ~ N_Child_HH + H003

```{r}
glm_child <- glm(C003_01 ~ N_Child_HH + H003, family = "binomial", data = child_ica_dummy %>% filter(C001 >= 5, !is.na(H003), DID == 266))
...
step(glm_child_null, direction = "both",
     scope = (~ C001 + C002_01 + N_Child_HH + H003))
```
...
#### Hunza. GLM Age >= 5 C003_01 ~ N_Child_HH + PR004ANDPR009_01

```{r}
glm_child <- glm(C003_01 ~ N_Child_HH + PR004ANDPR009_01, family = "binomial", data = child_ica_dummy %>% filter(C001 >= 5, DID == 266))
...
```{r}
step(glm_child_null, direction = "both", 
     scope = (~ C001 + C002_01 + PR004ANDPR009_01 + PR004ORPR009_01 + N_Child_HH))
```
...
#### Hunza. GLM Age >= 5 C003_01 ~ N_Child_HH + PR004ORPR009_01

```{r}
glm_child <- glm(C003_01 ~ N_Child_HH + PR004ORPR009_01, family = "binomial", data = child_ica_dummy %>% filter(C001 >= 5, DID == 266))
...
#### Each Dist. GLM Age >= 5 C003_01 ~ N_Child_HH + H003
...
each_dist <- sapply(as.numeric(as.character(unique(child_ica_dummy$DID))), function(id){
  ...
  glm_child <- glm(C003_01 ~ N_Child_HH + H003, 
...
#### Whole Pak. GLM Age >= 5 C003_01 ~ N_Child_HH + PR004ANDPR009_01 + relevel(DID, ref = "266")

```{r}
glm_child <- glm(C003_01 ~ N_Child_HH + PR004ANDPR009_01 + relevel(DID, ref = "266"), family = "binomial", data = child_ica_dummy %>% filter(C001 >= 5))
...
#### Each Dist. GLM Age >= 5 C003_01 ~ N_Child_HH + PR004ANDPR009_01

```{r}
each_dist <- sapply(as.numeric(as.character(unique(child_ica_dummy$DID))), function(id){
  child_ica_dummy <- child_ica_dummy %>% filter(DID == id)
  glm_child <- glm(C003_01 ~ N_Child_HH + PR004ANDPR009_01, 
...
#### Not dists_not_fit. GLM Age >= 5 C003_01 ~ N_Child_HH + PR004ANDPR009_01

```{r}
glm_child <- glm(C003_01 ~ N_Child_HH + PR004ANDPR009_01, family = "binomial", data = child_ica_dummy %>% filter(C001 >= 5, !DID %in% dists_not_fit))
