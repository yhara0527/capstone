...
```{r}
# child_ica_dummy %>%
#   mosaicplot( ~ School.Enrollment + Gender + Num.of.Child,
#        highlighting = "School.Enrollment", highlighting_fill = c("lightblue", "pink"),
#        direction = c("v", "h", "h"))
```
...
```{r}
prcomp(child_ica_dummy %>% 
         select(School.Enrollment, Gender, Num.of.Child))
rpca <- child_ica_dummy %>% 
  select(School.Enrollment, Gender, Num.of.Child) %>% 
  mutate(School.Enrollment = as.numeric(School.Enrollment),
         Gender = as.numeric(Gender),
         Num.of.Child = as.numeric(Num.of.Child)) %>% 
  prcomp()
...
```{r}
child_ica_dummy %>% mean(School.Enrollment)
ica %>% 
  mutate(centroid = st_centroid(geometry),
    x = st_coordinates(centroid)[,1],
    y = st_coordinates(centroid)[,2]) %>% 
  ...
  geom_point(data = child_ica_dummy %>% 
               ...
               group_by(DID) %>% 
               group_by(Gender) %>% 
               mutate(rate= .$School.Enrollment %>% mean()) %>% 
               ungroup %>% 
               summarize(x = unique(x),
                         y = unique(y),
                         DID = unique(DID),
...
temp_data <- child_ica_dummy %>% 
  filter(!is.na(x),Gender==1) %>%
  group_by(DID) %>%
  summarise(rate=mean(School.Enrollment),
            x=unique(x),
            y=unique(y),
            DNAME=unique(DNAME))

pal <- colorNumeric(palette="Blues", domain=temp_data$rate)


leaflet() %>% 
  addTiles() %>% 
  setView(lat=mean(co$y), lng=mean(co$x), zoom = 5)%>% 
  addCircleMarkers(~temp_data$x, ~temp_data$y, label=paste(temp_data$DNAME,temp_data$rate),color=~pal(temp_data$rate), stroke=FALSE, data = temp_data) %>% 
